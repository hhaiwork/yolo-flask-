...
class LoadImages:
    ...
    def load_info(self):
        ...
        # letterbox函数来对输入图片进行缩放操作，以适应网络的输入尺寸，并将其存储为img变量。其中，img0是原始图像，self.img_size是目标尺寸大小，stride是步长的大小。
        img = letterbox(img0, self.img_size, stride=self.stride)[0]
        # 该代码段主要实现了将读取的BGR格式的图像数据转换为RGB格式，并且调整了图像的维度。其中，img是读取到的原始图像数据，[:, :, ::-1]表示将BGR格式的通道顺序调整为RGB格式，
        # transpose(2, 0, 1)表示将图像数据的维度调整为(3,416,416)的形式。
        img = img[:, :, ::-1].transpose(2, 0, 1)
        # 该代码段中，np.ascontiguousarray()函数将输入的数组img进行内存连续性处理并返回。即如果输入的数组不是内存连续的，该函数会把它变成内存连续的数组。
        img = np.ascontiguousarray(img)
        return img, img0, self.cap


# 将预测框的坐标从缩放后的图像大小转换回原图大小
#   如果没有给定 ratio_pad 参数，则从 img0_shape 中计算出 gain，并计算出相应的 wh padding；
#     如果给定了 ratio_pad 参数，则从中取出对应的 gain 和 padding；
#     利用获得的 gain 和 padding 对预测框坐标进行变换，得到其在原图中的坐标；
#     利用 clip_coords 函数将坐标限制在原图的大小范围内；
#     返回转换后的坐标。
